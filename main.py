...
def summon_enigma_machine(mode = 10, initial_text = '', initial_rotor = ''):
    """
    Эта функция должна спрашивать параметры, для запуска "машины".
    В свою очередь машина состоит из нескольких функций.
    param :mode: определяет режим работы вывода текста. 0 - неизместные символы остаются на месте.
    1 - пропуск неизвестных символов (в том числе знаков препинания).
    param :initial_text: текст на вход. Если текст не был передан функции, то она спросит его сама.
    param :initial_rotor: роторы на вход. Тут я даю возможность определить положение роторов заранее (пока не реализовано)
    """
    ...
    while rotor != 0:  # всегда не =
        print('Введите назнание ротора (без расширения .txt, для остановки отправьте пробел):')
        rotor = str(input())
        if rotor != ' ':
    ...
    for i in range(len(temp_rotor_pos)):
        ...
        # заносим ротор из файла в псевдомассив
        line_count = 0
        for line in lines:
